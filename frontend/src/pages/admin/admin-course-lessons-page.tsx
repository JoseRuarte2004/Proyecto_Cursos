...
import { Download, Paperclip, PencilLine, Plus, Trash2, X } from "lucide-react";
...
export function AdminCourseLessonsPage() {
  ...
  const courseWasRemoved =
    lessonsQuery.isError && lessonsQuery.error.message === "course not found";

  useEffect(() => {
    if (!courseWasRemoved || !courseId) {
      return;
    }

    removeRecentAdminCourse(courseId);
  }, [courseId, courseWasRemoved]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      createForm.reset();
      void queryClient.invalidateQueries({ queryKey: ["admin-lessons", courseId] });
    },
  ...
  return (
    <div className="space-y-8">
      <PageIntro
        ...
        title={`Clases del curso ${courseId}`}
        description="Cargá material, ordená el recorrido y ajustá los links de video desde un CRUD directo."
        actions={
      ...
      {lessonsQuery.isLoading ? (
        ...
        <EmptyState
          title="Todavía no hay classes"
          description="Agregá las primeras lessons para que el aula tenga contenido."
          action={
            <Button onClick={() => setCreateOpen(true)}>
              <Plus className="h-4 w-4" />
              Crear la primera
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Suma imagenes, videos o archivos complementarios a la clase.
            </p>
          </div>
